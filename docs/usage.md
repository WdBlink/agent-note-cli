...
`--source all|codex|claude|cursor` 控制启用的会话来源。默认启用三个来源；如果只有其中一种工具，请明确选择它。Cursor 读取 `~/.cursor/projects/*/agent-transcripts/` 下的主会话和子 Agent；生成、刷新和深读会调用对应来源的模型 CLI（Cursor 使用 Cursor Agent CLI `agent`）。
...
一是 schema 只能写在 prompt 里。实测 Cursor 会在完整对象后多吐一个 `}`，也会把 schema 钉死的标识符重新拼写；这两类传输层偏差由 CLI 侧修正（读取第一个完整 JSON 值、把只有唯一合法值的 enum 还原），内容本身不做改写，仍由原有 Zod 契约校验。

二是每次整理调用都会在 `~/.cursor/projects/` 下留一个以临时工作目录命名的 chat（形如 `...-structured-today-call-XXXXXX`），Cursor 没有 `--ephemeral` 之类的参数可以关掉。扫描已排除这些目录，它们不会变成证据、也不占用发现预算，但会占磁盘。需要清理时删掉这些目录即可，它们不含你的工作记录：

```sh
ls -d ~/.cursor/projects/*structured-today-call*   # 先看
rm -rf ~/.cursor/projects/*structured-today-call*  # 再删
```
...
`--root` 可重复。原后端依赖目录名中的 `codex`、`claude` 或 `cursor` 识别来源；无该标识的目录会被拒绝。命令行不根据 `CODEX_HOME`、`CLAUDE_CONFIG_DIR` 自动改写 App 默认扫描目录，请用 `--root` 或 settings 明确设置。
...
选择 Cursor 整理时，证据还会经过 `~/.cursor/mcp.json` 中已配置的 MCP server：Cursor Agent CLI 没有忽略用户级配置的参数，Agent Note 无法在调用时关掉它们。
...
**Cursor 整理反复失败**：多半是返回值不满足 schema。Cursor 没有 CLI 层的结构化输出约束，换一个更强的模型（`--cursor-model`）通常比重试有效；也可以改用 `--source codex` 或 `--source claude` 整理同一批会话。
