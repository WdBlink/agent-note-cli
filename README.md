...
```text
  ...
  移动(↑↓/jk)  打开(Enter)  返回(q)  退出(Esc)
```

| 场景 | 按键 |
| --- | --- |
| 菜单 | `↑↓` / `j k` 移动，`Enter` 打开，`1–9` 直接选择 |
| 返回与退出 | 子页面按 `q` 返回；首页连续按两次 `q` 退出；任意页面连续按两次 `Esc` 或 `Ctrl+C` 退出（底部不显示 `Ctrl+C`） |
| 搜索列表 | `/` 输入，`Enter` 筛选，`Backspace` 删除 |
| 阅读长文 | `↑↓` 滚动，`Space` / `PageDown` 下翻，`PageUp` 上翻，`Home` / `End` 跳转 |
| 工作线 / 深读 | `o` 直达会话，`s` 查看冻结来源，`e` 导出；工作线中 `d` 深读 |
| 来源列表 / 原文 | `Enter` 阅读原文，`o` 直达选中来源的会话 |
| 生成中 | `q` 取消并返回；连续按两次 `Esc` 或 `Ctrl+C` 退出 |

宽终端会显示选中项预览，窄终端自动切成单列。阅读位置和列表选择会在返回后保留。按 `?` 可在应用内查看完整键位。

v0.6.0 提供实验性的[会话直达](docs/session-jump.md)：复用已定位的 Codex app 或终端窗口，多个来源先选择。无法确认目标时显示原因，不执行 `resume`、新建会话或发送消息。
...
频繁启动时可使用 `agent-note ui --skip-intro` 跳过书本打开动画，直接进入首页；默认启动体验保持不变。
