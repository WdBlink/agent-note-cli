...
const help = `Agent Note CLI · 与 Agent Notebook 相同的 Today 后端
  ...
  --source all|codex|claude|cursor     会话来源，默认 all
  --compiler codex|claude|cursor      整理用的模型 CLI，默认跟随来源
  --root PATH                        扫描目录，可重复；目录名需含 codex、claude 或 cursor
...
async function main() {
  ...
  if (v.source && !['all', 'codex', 'claude', 'cursor'].includes(v.source)) throw new Error('--source 必须为 all、codex、claude 或 cursor。');
