...
export async function brief(options, dependencies = {}) {
  ...
  if (roots.some(r => !/codex|claude|cursor/i.test(r))) throw new Error('会话目录名称须包含 codex、claude 或 cursor，以便原后端识别来源。');
  if (!Array.isArray(settings.enabledSessionProviders) || !settings.enabledSessionProviders.length || settings.enabledSessionProviders.some(p => !['codex', 'claude', 'cursor'].includes(p))) throw new Error('请选择 Codex、Claude 或 Cursor 会话来源。');
  ...
  if (options.compiler && !['codex', 'claude', 'cursor'].includes(options.compiler)) throw new Error('--compiler 必须为 codex、claude 或 cursor。');
